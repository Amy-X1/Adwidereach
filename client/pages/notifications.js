    ...
    <DashboardLayout>
      ...
      <section className="section">
        <div className="flex flex-wrap items-center justify-between gap-3 mb-6">
          <div><h1 className="text-2xl font-bold">Notifications</h1><p className="mt-1 text-sm text-gray-500">Updates about your orders, payments, support tickets — and announcements from the team.</p></div>
          ...
          {notifications.length === 0 ? <p className="py-8 text-center text-sm text-gray-500">No notifications yet.</p> : notifications.map((notification) => (
            <div key={notification.id} className={`border-b py-4 last:border-0 ${notification.isRead ? '' : 'bg-brand-50 dark:bg-gray-900'}`}>
              <div className="flex items-start justify-between gap-4">
                <div>
                  <p className="flex flex-wrap items-center gap-2 font-medium">{notification.type === 'ANNOUNCEMENT' && <span className="badge bg-amber-100 text-amber-700 dark:bg-amber-900/40 dark:text-amber-300">Announcement</span>}{notification.title}</p><p className="mt-1 whitespace-pre-wrap text-sm text-gray-600 dark:text-gray-300">{notification.message}</p><p className="mt-1 text-xs text-gray-500">{new Date(notification.createdAt).toLocaleString()}</p></div>
